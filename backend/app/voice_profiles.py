...
sample_text (replacing what was there) and distils them once into style_notes.
An optional tone_notes field lets the agent add short style guidance.

Generation reads style_notes, never sample_text: the samples advertise the
agent's other properties, and showing them to the generator carried their
facts into new listings (docs/VOICE-CONTAMINATION.md). sample_text is kept
only so the agent can see what they uploaded.
"""
...
def get_voice_profile(
    ...
    with closing(db.connect()) as conn:
        row = conn.execute(
            "SELECT sample_text, style_notes, tone_notes, updated_at"
            " FROM voice_profiles WHERE user_id = ?",
            (user_id,),
        ).fetchone()
    if row is None:
        return VoiceProfile(
            sample_text="", style_notes="", tone_notes="", updated_at=None
        )
...
async def put_voice_profile(
    ...
    New samples are distilled into style_notes here, so the one LLM call this
    costs happens on upload rather than on every generation.
    """
    ...
    style_notes = None if sample_text is None else generation.extract_style(sample_text)

    with closing(db.connect()) as conn:
        existing = conn.execute(
            "SELECT sample_text, style_notes, tone_notes FROM voice_profiles"
            " WHERE user_id = ?",
            (user_id,),
        ).fetchone()
        ...
        new_style = style_notes if style_notes is not None else (
            existing["style_notes"] if existing else ""
        )
        ...
        conn.execute(
            ...
            " (user_id, sample_text, style_notes, tone_notes, updated_at)"
            " VALUES (?, ?, ?, ?, datetime('now'))"
            ...
            (user_id, new_sample, new_style, new_notes),
        )
        conn.commit()
        row = conn.execute(
            "SELECT sample_text, style_notes, tone_notes, updated_at"
