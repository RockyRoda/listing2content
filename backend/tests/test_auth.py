...
def test_signup_duplicate_email_conflicts(client):
    ...
    resp = client.post("/api/auth/signup", json={"email": "a@b.com", "password": "pw2"})
    assert resp.status_code == 409


def test_signup_rejects_invalid_email(client):
    resp = client.post("/api/auth/signup", json={"email": "not-an-email", "password": "pw"})
    assert resp.status_code == 422


def test_signin_success_returns_token(client):
    client.post("/api/auth/signup", json={"email": "a@b.com", "password": "pw"})
    resp = client.post("/api/auth/signin", json={"email": "a@b.com", "password": "pw"})
    assert resp.status_code == 200
...
def test_signin_wrong_password_unauthorized(client):
    client.post("/api/auth/signup", json={"email": "a@b.com", "password": "pw"})
    resp = client.post("/api/auth/signin", json={"email": "a@b.com", "password": "nope"})
    assert resp.status_code == 401


def test_signin_unknown_email_unauthorized(client):
    resp = client.post("/api/auth/signin", json={"email": "ghost@b.com", "password": "pw"})
    assert resp.status_code == 401


def test_me_with_valid_token(client):
    token = client.post(
        "/api/auth/signup", json={"email": "a@b.com", "password": "pw"}
    ).json()["token"]
